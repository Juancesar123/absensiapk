...
export class AprovalregisterProvider {
  ...
  getdata() : Observable<userModel[]> {
    ...
    let options = new RequestOptions({headers:header});
     return this.http.get("http://198.50.174.117/users",options).map(res => res.json().data as userModel[]);
  }
  ubahstatus(data,id){
    let header = new Headers({'content-Type':'application/json','Authorization':localStorage.getItem('token')});
    let options = new RequestOptions({headers:header});
    let body = JSON.stringify(data)
     return this.http.patch("http://198.50.174.117/users/"+id,body,options).map(res => res.json().data as userModel[]);
  }
}
