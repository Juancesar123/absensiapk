...
export class UseraccessProvider {
  ...
  simpandata(data){
    ...
    return this.http.post('http://localhost:3030/masteruseraccess',body,options).map(res => res.json() as UserAccessModel[]);
  }
  hapusdata(item){
    let header = new Headers({'Content-Type':'Application/json','Authorization':localStorage.getItem('token')});
    let options = new RequestOptions({headers:header});
    let id = item.id
    return this.http.delete('http://localhost:3030/masteruseraccess/'+id,options).map(res => res.json() as UserAccessModel[]);
  }
  updatedata(data,id){
    let header = new Headers({'Content-Type':'Application/json','Authorization':localStorage.getItem('token')});
    let options = new RequestOptions({headers:header});
    let body = JSON.stringify(data);
    return this.http.patch('http://localhost:3030/masteruseraccess/'+id,body,options).map(res => res.json() as UserAccessModel[]);
